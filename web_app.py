...
def detect_code_length(series):
    ...
    # Get the most common length that's >= 6
    length_counts = lengths.value_counts()
    valid_lengths = length_counts[length_counts.index >= 6]
    
    if len(valid_lengths) > 0:
        # Return the maximum common length (for HS codes, longer is more specific)
...
def clean_hs_code(code, target_length=None):
    ...
    if 'e' in code.lower():
        try:
            num = int(float(code))
            code = str(num)
            # Pad to target length to restore leading zeros
            if target_length and len(code) < target_length:
                code = code.zfill(target_length)
        except:
            pass
    
    # Handle decimals (e.g., "123456.0")
    if '.' in code:
        try:
            num = int(float(code))
            code = str(num)
...
def clean_hs_code_vectorized(series, target_length=None):
    ...
    # Pad short codes that might have lost leading zeros
    def pad_if_needed(x):
...
def process_tree_data(df, market_name, selected_column=None, expected_length=None):
    ...
    # Detect code length if not specified
    ...
    # Use vectorized cleaning with target length
...
def create_executive_summary_excel(selected_markets, tree_counts, pairwise_stats, pairwise_results, digit_choice, comprehensive_df=None):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        green_format = workbook.add_format({
            'bg_color': '#92D050', 'border': 1, 'align': 'center'
        })
        ...
        # Section 2: FIXED Match Analysis
        row += 2
        ws.merge_range(row, 0, row, 6, "MATCH ANALYSIS (CORRECTED)", section_format)
        ...
        for pair_key, stats in pairwise_stats.items():
            ...
            # FIXED: Overall match = shared / total prefixes
...
# =========================
    ...
    if len(st.session_state.trees) < 2:
        ...
        **🔧 Fixed Issues:**
        ...
        # Show code lengths for each tree
        ...
        **{digit_choice}-digit matching example:**
        ...
        if 'multi_tree_results' in st.session_state:
            ...
            # FIXED: Correlation Analysis with correct percentages
            st.markdown("#### 🔄 Correlation Analysis (FIXED)")
            ...
            for pair_key, stats in pairwise_stats.items():
                ...
                # FIXED: Overall match = shared / total
                overall_pct = (total_shared / total_prefixes * 100) if total_prefixes > 0 else 0
                # 1:1 of shared
                shared_pct = (one_to_one / total_shared * 100) if total_shared > 0 else 0
            ...
            **📝 Percentage Explanation:**
            - **Overall Match %** = Shared Prefixes / Total Prefixes (includes "Only" codes)
            - **1:1 of Shared %** = One-to-One / Shared Prefixes (only shared codes)
            """)
            ...
            if view_tab == "🌐 Comprehensive (All MPs)":
                st.markdown("""
                **Comprehensive View** shows each prefix with:
                - **Source_MP**: Which marketplace(s) have this code
                - **Match_Status**: Match (All MPs), Partial, or MP-Only
                """)
                ...
                if selected_pair:
                    ...
                    with col1:
                        filter_options = [
                            ...
                            "Many-to-One", 
                            "Many-to-Many",
                            f"No Match ({market_a}-Only)",
                            f"No Match ({market_b}-Only)",
                            "All No Match"
                        ]
                    ...
                    with col2:
                        filter_source = st.selectbox(
                            "Filter by Source MP",
                            ["All", market_a, market_b, "Both"],
                            key="pairwise_source_filter"
                        )
                    with col3:
    ...
    if len(st.session_state.trees) < 1:
        ...
        if asin_file:
            ...
            if asin_raw is not None:
                ...
                # Detect expected length from ASIN file
                ...
                if source_market and target_markets:
                    if st.button("🚀 Run Mapping", type="primary"):
                        ...
                        # Clean with proper length handling
...
# =========================
# DIAGNOSE ISSUES (ENHANCED)
# =========================
elif mode == "🔍 Diagnose Issues":
    st.title("🔍 Diagnose Issues")
    
    st.markdown("""
    ### Debug HS Code Matching
    
    Use this to verify why a specific code might not be matching correctly.
    """)
    ...
    if len(st.session_state.trees) < 1:
        ...
        if test_code:
            ...
            # Show how the code is processed
            ...
            with col2:
                cleaned = clean_hs_code(test_code, 12)  # Try with 12-digit padding
            ...
            for market in markets:
                ...
                # Clean with market's code length
                ...
                with st.expander(f"🏪 {market} ({code_len}-digit codes)"):
                    ...
                    # Show some codes from this tree that start similarly
